...
pub fn get_config_path() -> Result<PathBuf, ConfigDirectoryError> {
    let dirs = ProjectDirs::from("com", "rgamesync", "rgamesync")
        .ok_or(ConfigDirectoryError::NoConfigDir)?;

    let config_dir = dirs.config_dir();
    create_dir_all(config_dir)
        .map_err(|_err| ConfigDirectoryError::CouldNotMakeConfigDir(config_dir.to_owned()))?;
